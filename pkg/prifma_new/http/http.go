...
func (t *Http) HandleRequest(result prifma_new.HandleRequestResult) (prifma_new.HandleRequestResult, error) {
	...
	if err != nil {
		if err == prifma_new.ErrOutgoingIpNotDefined {
			result.SetResponse(prifma_new.NewResponseError(http.StatusBadRequest, err.Error()))
		} else {
			result.SetResponse(prifma_new.NewResponseError(http.StatusBadGateway, err.Error()))
		}
...
func (t *Http) Clone() prifma_new.Module {
	return t
}
